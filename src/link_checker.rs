...
const URL_REQUEST_RESPONSE:usize = 1;
const URL:usize = 0;

pub(crate) async fn check_links(input_path:String, threads: usize, output_path: String)->Result<bool, Error>{
    ...
    for url_result in  line_processing_result.iter(){
        match url_result{
            Ok(data) => {
                let title_url = data;
                if title_url.0.is_empty(){continue;}
            ...
            Err(_error) => {
                return Err(Error::new(std::io::ErrorKind::Unsupported, "unexpected error"))
            }
        }
